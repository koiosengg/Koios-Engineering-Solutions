import React from "react";
import Star from "../../assets/ThreeDprinting/Testimony/Star.webp";

function Testimony() {
  return (
    <div className="threeD-printing-testimony">
      <h2>
        What <span> Our Customers</span> Say
      </h2>
      <div className="threeD-printing-testimony-container">
        <div className="threeD-printing-testimony-wrapper">
          <div className="threeD-printing-testimony-marquee">
            <div className="threeD-printing-testimony-set">
              <section>
                <img src={Star} alt="" />
                <img src={Star} alt="" />
                <img src={Star} alt="" />
                <img src={Star} alt="" />
                <img src={Star} alt="" />
              </section>
              <p>
                Lorem ipsum dolor sit amet consectetur. Tellus est eget nunc
                nunc viverra molestie arcu. Quis id adipiscing est.
              </p>
              <h3>Thomas W</h3>
            </div>
            <div className="threeD-printing-testimony-set">
              <section>
                <img src={Star} alt="" />
                <img src={Star} alt="" />
                <img src={Star} alt="" />
                <img src={Star} alt="" />
                <img src={Star} alt="" />
              </section>
              <p>
                Lorem ipsum dolor sit amet consectetur. Tellus est eget nunc
                nunc viverra molestie arcu. Quis id adipiscing est.
              </p>
              <h3>Thomas W</h3>
            </div>
            <div className="threeD-printing-testimony-set">
              <section>
                <img src={Star} alt="" />
                <img src={Star} alt="" />
                <img src={Star} alt="" />
                <img src={Star} alt="" />
                <img src={Star} alt="" />
              </section>
              <p>
                Lorem ipsum dolor sit amet consectetur. Tellus est eget nunc
                nunc viverra molestie arcu. Quis id adipiscing est.
              </p>
              <h3>Thomas W</h3>
            </div>
            <div className="threeD-printing-testimony-set">
              <section>
                <img src={Star} alt="" />
                <img src={Star} alt="" />
                <img src={Star} alt="" />
                <img src={Star} alt="" />
                <img src={Star} alt="" />
              </section>
              <p>
                Lorem ipsum dolor sit amet consectetur. Tellus est eget nunc
                nunc viverra molestie arcu. Quis id adipiscing est.
              </p>
              <h3>Thomas W</h3>
            </div>
            <div className="threeD-printing-testimony-set">
              <section>
                <img src={Star} alt="" />
                <img src={Star} alt="" />
                <img src={Star} alt="" />
                <img src={Star} alt="" />
                <img src={Star} alt="" />
              </section>
              <p>
                Lorem ipsum dolor sit amet consectetur. Tellus est eget nunc
                nunc viverra molestie arcu. Quis id adipiscing est.
              </p>
              <h3>Thomas W</h3>
            </div>
            <div className="threeD-printing-testimony-set">
              <section>
                <img src={Star} alt="" />
                <img src={Star} alt="" />
                <img src={Star} alt="" />
                <img src={Star} alt="" />
                <img src={Star} alt="" />
              </section>
              <p>
                Lorem ipsum dolor sit amet consectetur. Tellus est eget nunc
                nunc viverra molestie arcu. Quis id adipiscing est.
              </p>
              <h3>Thomas W</h3>
            </div>
            <div className="threeD-printing-testimony-set">
              <section>
                <img src={Star} alt="" />
                <img src={Star} alt="" />
                <img src={Star} alt="" />
                <img src={Star} alt="" />
                <img src={Star} alt="" />
              </section>
              <p>
                Lorem ipsum dolor sit amet consectetur. Tellus est eget nunc
                nunc viverra molestie arcu. Quis id adipiscing est.
              </p>
              <h3>Thomas W</h3>
            </div>
            <div className="threeD-printing-testimony-set">
              <section>
                <img src={Star} alt="" />
                <img src={Star} alt="" />
                <img src={Star} alt="" />
                <img src={Star} alt="" />
                <img src={Star} alt="" />
              </section>
              <p>
                Lorem ipsum dolor sit amet consectetur. Tellus est eget nunc
                nunc viverra molestie arcu. Quis id adipiscing est.
              </p>
              <h3>Thomas W</h3>
            </div>
            <div className="threeD-printing-testimony-set">
              <section>
                <img src={Star} alt="" />
                <img src={Star} alt="" />
                <img src={Star} alt="" />
                <img src={Star} alt="" />
                <img src={Star} alt="" />
              </section>
              <p>
                Lorem ipsum dolor sit amet consectetur. Tellus est eget nunc
                nunc viverra molestie arcu. Quis id adipiscing est.
              </p>
              <h3>Thomas W</h3>
            </div>
            <div className="threeD-printing-testimony-set">
              <section>
                <img src={Star} alt="" />
                <img src={Star} alt="" />
                <img src={Star} alt="" />
                <img src={Star} alt="" />
                <img src={Star} alt="" />
              </section>
              <p>
                Lorem ipsum dolor sit amet consectetur. Tellus est eget nunc
                nunc viverra molestie arcu. Quis id adipiscing est.
              </p>
              <h3>Thomas W</h3>
            </div>
            <div className="threeD-printing-testimony-set">
              <section>
                <img src={Star} alt="" />
                <img src={Star} alt="" />
                <img src={Star} alt="" />
                <img src={Star} alt="" />
                <img src={Star} alt="" />
              </section>
              <p>
                Lorem ipsum dolor sit amet consectetur. Tellus est eget nunc
                nunc viverra molestie arcu. Quis id adipiscing est.
              </p>
              <h3>Thomas W</h3>
            </div>
            <div className="threeD-printing-testimony-set">
              <section>
                <img src={Star} alt="" />
                <img src={Star} alt="" />
                <img src={Star} alt="" />
                <img src={Star} alt="" />
                <img src={Star} alt="" />
              </section>
              <p>
                Lorem ipsum dolor sit amet consectetur. Tellus est eget nunc
                nunc viverra molestie arcu. Quis id adipiscing est.
              </p>
              <h3>Thomas W</h3>
            </div>
            <div className="threeD-printing-testimony-set">
              <section>
                <img src={Star} alt="" />
                <img src={Star} alt="" />
                <img src={Star} alt="" />
                <img src={Star} alt="" />
                <img src={Star} alt="" />
              </section>
              <p>
                Lorem ipsum dolor sit amet consectetur. Tellus est eget nunc
                nunc viverra molestie arcu. Quis id adipiscing est.
              </p>
              <h3>Thomas W</h3>
            </div>
            <div className="threeD-printing-testimony-set">
              <section>
                <img src={Star} alt="" />
                <img src={Star} alt="" />
                <img src={Star} alt="" />
                <img src={Star} alt="" />
                <img src={Star} alt="" />
              </section>
              <p>
                Lorem ipsum dolor sit amet consectetur. Tellus est eget nunc
                nunc viverra molestie arcu. Quis id adipiscing est.
              </p>
              <h3>Thomas W</h3>
            </div>
            <div className="threeD-printing-testimony-set">
              <section>
                <img src={Star} alt="" />
                <img src={Star} alt="" />
                <img src={Star} alt="" />
                <img src={Star} alt="" />
                <img src={Star} alt="" />
              </section>
              <p>
                Lorem ipsum dolor sit amet consectetur. Tellus est eget nunc
                nunc viverra molestie arcu. Quis id adipiscing est.
              </p>
              <h3>Thomas W</h3>
            </div>
            <div className="threeD-printing-testimony-set">
              <section>
                <img src={Star} alt="" />
                <img src={Star} alt="" />
                <img src={Star} alt="" />
                <img src={Star} alt="" />
                <img src={Star} alt="" />
              </section>
              <p>
                Lorem ipsum dolor sit amet consectetur. Tellus est eget nunc
                nunc viverra molestie arcu. Quis id adipiscing est.
              </p>
              <h3>Thomas W</h3>
            </div>
            <div className="threeD-printing-testimony-set">
              <section>
                <img src={Star} alt="" />
                <img src={Star} alt="" />
                <img src={Star} alt="" />
                <img src={Star} alt="" />
                <img src={Star} alt="" />
              </section>
              <p>
                Lorem ipsum dolor sit amet consectetur. Tellus est eget nunc
                nunc viverra molestie arcu. Quis id adipiscing est.
              </p>
              <h3>Thomas W</h3>
            </div>
            <div className="threeD-printing-testimony-set">
              <section>
                <img src={Star} alt="" />
                <img src={Star} alt="" />
                <img src={Star} alt="" />
                <img src={Star} alt="" />
                <img src={Star} alt="" />
              </section>
              <p>
                Lorem ipsum dolor sit amet consectetur. Tellus est eget nunc
                nunc viverra molestie arcu. Quis id adipiscing est.
              </p>
              <h3>Thomas W</h3>
            </div>
          </div>
        </div>
        <div className="threeD-printing-testimony-wrapper">
          <div className="threeD-printing-testimony-marquee threeD-printing-testimony-marquee-rev">
            <div className="threeD-printing-testimony-set">
              <section>
                <img src={Star} alt="" />
                <img src={Star} alt="" />
                <img src={Star} alt="" />
                <img src={Star} alt="" />
                <img src={Star} alt="" />
              </section>
              <p>
                Lorem ipsum dolor sit amet consectetur. Tellus est eget nunc
                nunc viverra molestie arcu. Quis id adipiscing est.
              </p>
              <h3>Thomas W</h3>
            </div>
            <div className="threeD-printing-testimony-set">
              <section>
                <img src={Star} alt="" />
                <img src={Star} alt="" />
                <img src={Star} alt="" />
                <img src={Star} alt="" />
                <img src={Star} alt="" />
              </section>
              <p>
                Lorem ipsum dolor sit amet consectetur. Tellus est eget nunc
                nunc viverra molestie arcu. Quis id adipiscing est.
              </p>
              <h3>Thomas W</h3>
            </div>
            <div className="threeD-printing-testimony-set">
              <section>
                <img src={Star} alt="" />
                <img src={Star} alt="" />
                <img src={Star} alt="" />
                <img src={Star} alt="" />
                <img src={Star} alt="" />
              </section>
              <p>
                Lorem ipsum dolor sit amet consectetur. Tellus est eget nunc
                nunc viverra molestie arcu. Quis id adipiscing est.
              </p>
              <h3>Thomas W</h3>
            </div>
            <div className="threeD-printing-testimony-set">
              <section>
                <img src={Star} alt="" />
                <img src={Star} alt="" />
                <img src={Star} alt="" />
                <img src={Star} alt="" />
                <img src={Star} alt="" />
              </section>
              <p>
                Lorem ipsum dolor sit amet consectetur. Tellus est eget nunc
                nunc viverra molestie arcu. Quis id adipiscing est.
              </p>
              <h3>Thomas W</h3>
            </div>
            <div className="threeD-printing-testimony-set">
              <section>
                <img src={Star} alt="" />
                <img src={Star} alt="" />
                <img src={Star} alt="" />
                <img src={Star} alt="" />
                <img src={Star} alt="" />
              </section>
              <p>
                Lorem ipsum dolor sit amet consectetur. Tellus est eget nunc
                nunc viverra molestie arcu. Quis id adipiscing est.
              </p>
              <h3>Thomas W</h3>
            </div>
            <div className="threeD-printing-testimony-set">
              <section>
                <img src={Star} alt="" />
                <img src={Star} alt="" />
                <img src={Star} alt="" />
                <img src={Star} alt="" />
                <img src={Star} alt="" />
              </section>
              <p>
                Lorem ipsum dolor sit amet consectetur. Tellus est eget nunc
                nunc viverra molestie arcu. Quis id adipiscing est.
              </p>
              <h3>Thomas W</h3>
            </div>
            <div className="threeD-printing-testimony-set">
              <section>
                <img src={Star} alt="" />
                <img src={Star} alt="" />
                <img src={Star} alt="" />
                <img src={Star} alt="" />
                <img src={Star} alt="" />
              </section>
              <p>
                Lorem ipsum dolor sit amet consectetur. Tellus est eget nunc
                nunc viverra molestie arcu. Quis id adipiscing est.
              </p>
              <h3>Thomas W</h3>
            </div>
            <div className="threeD-printing-testimony-set">
              <section>
                <img src={Star} alt="" />
                <img src={Star} alt="" />
                <img src={Star} alt="" />
                <img src={Star} alt="" />
                <img src={Star} alt="" />
              </section>
              <p>
                Lorem ipsum dolor sit amet consectetur. Tellus est eget nunc
                nunc viverra molestie arcu. Quis id adipiscing est.
              </p>
              <h3>Thomas W</h3>
            </div>
            <div className="threeD-printing-testimony-set">
              <section>
                <img src={Star} alt="" />
                <img src={Star} alt="" />
                <img src={Star} alt="" />
                <img src={Star} alt="" />
                <img src={Star} alt="" />
              </section>
              <p>
                Lorem ipsum dolor sit amet consectetur. Tellus est eget nunc
                nunc viverra molestie arcu. Quis id adipiscing est.
              </p>
              <h3>Thomas W</h3>
            </div>
            <div className="threeD-printing-testimony-set">
              <section>
                <img src={Star} alt="" />
                <img src={Star} alt="" />
                <img src={Star} alt="" />
                <img src={Star} alt="" />
                <img src={Star} alt="" />
              </section>
              <p>
                Lorem ipsum dolor sit amet consectetur. Tellus est eget nunc
                nunc viverra molestie arcu. Quis id adipiscing est.
              </p>
              <h3>Thomas W</h3>
            </div>
            <div className="threeD-printing-testimony-set">
              <section>
                <img src={Star} alt="" />
                <img src={Star} alt="" />
                <img src={Star} alt="" />
                <img src={Star} alt="" />
                <img src={Star} alt="" />
              </section>
              <p>
                Lorem ipsum dolor sit amet consectetur. Tellus est eget nunc
                nunc viverra molestie arcu. Quis id adipiscing est.
              </p>
              <h3>Thomas W</h3>
            </div>
            <div className="threeD-printing-testimony-set">
              <section>
                <img src={Star} alt="" />
                <img src={Star} alt="" />
                <img src={Star} alt="" />
                <img src={Star} alt="" />
                <img src={Star} alt="" />
              </section>
              <p>
                Lorem ipsum dolor sit amet consectetur. Tellus est eget nunc
                nunc viverra molestie arcu. Quis id adipiscing est.
              </p>
              <h3>Thomas W</h3>
            </div>
            <div className="threeD-printing-testimony-set">
              <section>
                <img src={Star} alt="" />
                <img src={Star} alt="" />
                <img src={Star} alt="" />
                <img src={Star} alt="" />
                <img src={Star} alt="" />
              </section>
              <p>
                Lorem ipsum dolor sit amet consectetur. Tellus est eget nunc
                nunc viverra molestie arcu. Quis id adipiscing est.
              </p>
              <h3>Thomas W</h3>
            </div>
            <div className="threeD-printing-testimony-set">
              <section>
                <img src={Star} alt="" />
                <img src={Star} alt="" />
                <img src={Star} alt="" />
                <img src={Star} alt="" />
                <img src={Star} alt="" />
              </section>
              <p>
                Lorem ipsum dolor sit amet consectetur. Tellus est eget nunc
                nunc viverra molestie arcu. Quis id adipiscing est.
              </p>
              <h3>Thomas W</h3>
            </div>
            <div className="threeD-printing-testimony-set">
              <section>
                <img src={Star} alt="" />
                <img src={Star} alt="" />
                <img src={Star} alt="" />
                <img src={Star} alt="" />
                <img src={Star} alt="" />
              </section>
              <p>
                Lorem ipsum dolor sit amet consectetur. Tellus est eget nunc
                nunc viverra molestie arcu. Quis id adipiscing est.
              </p>
              <h3>Thomas W</h3>
            </div>
            <div className="threeD-printing-testimony-set">
              <section>
                <img src={Star} alt="" />
                <img src={Star} alt="" />
                <img src={Star} alt="" />
                <img src={Star} alt="" />
                <img src={Star} alt="" />
              </section>
              <p>
                Lorem ipsum dolor sit amet consectetur. Tellus est eget nunc
                nunc viverra molestie arcu. Quis id adipiscing est.
              </p>
              <h3>Thomas W</h3>
            </div>
            <div className="threeD-printing-testimony-set">
              <section>
                <img src={Star} alt="" />
                <img src={Star} alt="" />
                <img src={Star} alt="" />
                <img src={Star} alt="" />
                <img src={Star} alt="" />
              </section>
              <p>
                Lorem ipsum dolor sit amet consectetur. Tellus est eget nunc
                nunc viverra molestie arcu. Quis id adipiscing est.
              </p>
              <h3>Thomas W</h3>
            </div>
            <div className="threeD-printing-testimony-set">
              <section>
                <img src={Star} alt="" />
                <img src={Star} alt="" />
                <img src={Star} alt="" />
                <img src={Star} alt="" />
                <img src={Star} alt="" />
              </section>
              <p>
                Lorem ipsum dolor sit amet consectetur. Tellus est eget nunc
                nunc viverra molestie arcu. Quis id adipiscing est.
              </p>
              <h3>Thomas W</h3>
            </div>
          </div>
        </div>
      </div>
    </div>
  );
}

export default Testimony;
